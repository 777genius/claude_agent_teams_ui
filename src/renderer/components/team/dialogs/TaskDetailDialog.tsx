    ...
    return (
      <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
        <DialogContent className="sm:max-w-4xl">
          <DialogHeader>
            <DialogTitle>{t('dialogs.taskDetail.loadingTitle')}</DialogTitle>
          </DialogHeader>
          <div className="flex items-center gap-2 text-sm text-[var(--color-text-muted)]">
            <Loader2 className="size-4 animate-spin" />
            <span>{t('dialogs.taskDetail.fetchingTeam')}</span>
          </div>
        </DialogContent>
      </Dialog>
    );
  ...
  if (!currentTask) {
    return (
      <Dialog open={open} onOpenChange={(v) => !v && handleClose()}>
        <DialogContent className="sm:max-w-4xl">
          <DialogHeader>
            <DialogTitle>{t('dialogs.taskDetail.notFoundTitle')}</DialogTitle>
          </DialogHeader>
        </DialogContent>
      </Dialog>
    );
        ...
        <LightboxLockProvider value={setLightboxOpen}>
            ...
            <div className="flex min-w-0 items-center gap-2">
              {canReassign ? (
                ...
                <span className="text-xs italic text-[var(--color-text-muted)]">
                  {t('dialogs.taskDetail.unassigned')}
                </span>
              )}
            </div>
          ...
          {currentTask.needsClarification ? (
            ...
            >
              <span className="flex items-center gap-1.5">
                <HelpCircle size={14} />
                {currentTask.needsClarification === 'user'
                  ? t('dialogs.taskDetail.awaitingClarificationUser')
                  : t('dialogs.taskDetail.awaitingClarificationLead')}
              </span>
              <Button
                variant="ghost"
                size="sm"
              ...
              >
                {t('dialogs.taskDetail.markResolved')}
              </Button>
            </div>
          ) : null}
            ...
            <div className="space-y-2">
              ...
              {(relatedIds.length > 0 || relatedByIds.length > 0) && (
                <div className="flex items-center gap-1.5 text-xs font-medium text-[var(--color-text-muted)]">
                  <Link2 size={12} />
                  {t('dialogs.taskDetail.relatedTasksLabel')}
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-1.5">
                {relatedIds.length > 0 ? (
                  <div className="flex flex-wrap items-center gap-1.5">
                    <span className="text-xs text-[var(--color-text-muted)]">
                      {t('dialogs.taskDetail.linksLabel')}
                    </span>
                    {relatedIds.map((id) => {
                ...
                {relatedByIds.length > 0 ? (
                  <div className="flex flex-wrap items-center gap-1.5">
                    <span className="text-xs text-[var(--color-text-muted)]">
                      {t('dialogs.taskDetail.linkedFromLabel')}
                    </span>
                    {relatedByIds.map((id) => {
                      const depTask = taskMap.get(id);
                      const label = depTask
                ...
                {blockedByIds.length > 0 ? (
                  <div className="flex flex-wrap items-center gap-1.5">
                    <span className="inline-flex items-center gap-0.5 text-xs text-yellow-700 dark:text-yellow-300">
                      ...
                      {t('dialogs.taskDetail.blockedByLabel')}
                    </span>
                    {blockedByIds.map((id) => {
                      const depTask = taskMap.get(id);
                ...
                {blocksIds.length > 0 ? (
                  <div className="flex flex-wrap items-center gap-1.5">
                    <span className="inline-flex items-center gap-0.5 text-xs text-blue-600 dark:text-blue-400">
                      ...
                      {t('dialogs.taskDetail.blocksLabel')}
                    </span>
                    {blocksIds.map((id) => {
                      const depTask = taskMap.get(id);
                      const isCompleted = depTask?.status === 'completed';
          ...
          <div className="min-w-0 space-y-1">
            ...
            <CollapsibleTeamSection
              title={t('dialogs.taskDetail.descriptionTitle')}
              ...
              {editingDescription ? (
                <div className="space-y-2">
                  <TiptapEditor
                    ...
                    placeholder={t('dialogs.taskDetail.descriptionPlaceholder')}
                    autoFocus
                    minHeight="120px"
                    maxHeight="200px"
                    toolbar
                ...
                >
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent side="top">
                      {t('dialogs.taskDetail.editDescriptionTooltip')}
                    </TooltipContent>
                  </Tooltip>
                </div>
              ) : (
                <button
                  ...
                  onClick={startEditDescription}
                >
                  {t('dialogs.taskDetail.addDescription')}
                </button>
              )}
            </CollapsibleTeamSection>
            ...
            {/* Attachments */}
            <CollapsibleTeamSection
              title={t('dialogs.taskDetail.attachmentsTitle')}
            ...
            {variant === 'team' && canShowTaskChanges && onViewChanges ? (
              <CollapsibleTeamSection
                ...
                title={t('dialogs.taskDetail.changesTitle')}
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <button
                          ...
                          aria-label={t('dialogs.taskDetail.refreshAriaLabel')}
                        >
                          <RefreshCw
                            size={12}
                      ...
                      <TooltipContent side="top">
                        {t('dialogs.taskDetail.refreshTooltip')}
                      </TooltipContent>
                    </Tooltip>
                  ) : null
                }
              ...
              >
                {taskChangesLoading && (!taskChangesFiles || taskChangesFiles.length === 0) ? (
                  <div className="flex items-center gap-2 py-2 text-xs text-[var(--color-text-muted)]">
                    ...
                    {t('dialogs.taskDetail.loadingChanges')}
                  </div>
                ) : taskChangesError ? (
                      ...
                      >
                        ...
                        <span className="flex shrink-0 items-center gap-0.5 opacity-0 transition-opacity group-hover:opacity-100">
                          <Tooltip>
                            ...
                            <TooltipContent side="top">
                              {t('dialogs.taskDetail.reviewDiffTooltip')}
                            </TooltipContent>
                          </Tooltip>
                          {onOpenInEditor ? (
                            <Tooltip>
                              ...
                              <TooltipContent side="top">
                                {t('dialogs.taskDetail.openInEditorTooltip')}
                              </TooltipContent>
                            </Tooltip>
                          ) : null}
                        </span>
                  ...
                  <p className="text-xs text-[var(--color-text-muted)]">
                    {t('dialogs.taskDetail.noChanges')}
                  </p>
                ) : null}
              </CollapsibleTeamSection>
            ) : null}
            ...
            {variant === 'team' ? (
              <CollapsibleTeamSection
                ...
                title={t('dialogs.taskDetail.executionLogsTitle')}
                    ...
                    <span className="flex items-center gap-2 text-[10px] text-[var(--color-text-muted)]">
                      {executionPreviewOnline ? (
                        <span
                          ...
                          title={t('dialogs.taskDetail.onlineTitle')}
                        >
                ...
                <div className="flex items-center gap-2">
                  {kanbanTaskState.reviewer ? (
                    <span className="text-xs text-[var(--color-text-secondary)]">
                      {t('dialogs.taskDetail.reviewerLabel', { name: kanbanTaskState.reviewer })}
                    </span>
                  ) : null}
            ...
            {currentTask.historyEvents && currentTask.historyEvents.length > 0 ? (
              <CollapsibleTeamSection
                title={t('dialogs.taskDetail.workflowHistoryTitle')}
            ...
            {/* Comments */}
            <CollapsibleTeamSection
              title={t('dialogs.taskDetail.commentsTitle')}
    ...
    <div className="mt-3 space-y-1">
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-[11px] font-medium text-[var(--color-text-muted)]">
          {t('dialogs.taskDetail.fromComments')}
        </span>
      </div>
      ...
      {previewUrl ? (
        <ImageLightbox
          ...
          src={previewUrl}
          alt={t('dialogs.taskDetail.commentAttachment')}
        />
      ) : null}
    </div>
  );
