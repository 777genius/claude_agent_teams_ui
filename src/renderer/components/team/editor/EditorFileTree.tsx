...
export const EditorFileTree = ({
  ...
  if (loading && !fileTree) {
    return <div className="p-3 text-xs text-text-muted">{t('editor.fileTree.loadingFiles')}</div>;
  }

  if (treeNodes.length === 0) {
    return <div className="p-3 text-xs text-text-muted">{t('editor.fileTree.noFilesFound')}</div>;
  }
      ...
      <Dialog open={!!deleteConfirmPath} onOpenChange={(open) => !open && handleCancelDelete()}>
        <DialogContent className="w-96 max-w-96">
          <DialogHeader>
            ...
            <DialogDescription>
              {t('editor.fileTree.moveToTrashMessage', {
