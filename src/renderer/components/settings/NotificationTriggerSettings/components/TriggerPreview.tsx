    ...
    <div className="mt-4 border-t border-border pt-4">
      <div className="mb-2 flex items-center justify-between">
        <span className="text-xs uppercase tracking-widest text-text-muted">
          {t('settings.triggerSettings.preview')}
        </span>
        <button
        ...
        >
          {isLoading ? (
            <span className="flex items-center gap-1">
              ...
              {t('settings.triggerSettings.testing')}
            </span>
          ) : (
            t('settings.triggerSettings.testTrigger')
          )}
        </button>
      </div>
      ...
      {previewResult && !previewResult.loading && (
        <div className="space-y-2">
          <p className="text-sm text-text-secondary">
            ...
            {t('settings.triggerSettings.errorsDetected')}
          </p>
          ...
          {previewResult.truncated && (
            ...
            >
              ...
              <span>{t('settings.triggerSettings.searchStoppedEarly')}</span>
            </div>
          )}
          ...
          {previewResult.errors.slice(0, 10).map((error, idx) => (
            ...
            >
              ...
              >
                {t('settings.triggerSettings.viewSession')}
              </button>
            </div>
          ))}
          ...
          {effectiveCount > 10 && (
            <p className="text-xs text-text-muted">
              {t('settings.triggerSettings.andMore', { count: effectiveCount - 10 })}
            </p>
          )}
        </div>
      )}
