        ...
        <div className="space-y-3">
          <div>
            <div className="mb-1.5 flex items-center justify-between">
              ...
              >
                {allSelected
                  ? t('sidebar.taskFilters.clearAll')
                  : t('sidebar.taskFilters.selectAll')}
              </button>
            </div>
          ...
          <div>
            ...
            <Combobox
              ...
              placeholder={t('sidebar.taskFilters.allTeams')}
              searchPlaceholder={t('sidebar.taskFilters.searchTeams')}
              emptyMessage={t('sidebar.taskFilters.noTeamsFound')}
              className="text-[12px]"
            />
          </div>
          ...
          {projectOptions.length > 0 && (
            <div>
              <span className="mb-1.5 block text-[11px] font-semibold text-text-secondary">
                {t('sidebar.taskFilters.project')}
              </span>
              <Combobox
                options={projectOptions}
                value={draft.projectPath ?? ''}
                onValueChange={(v) => setDraft({ ...draft, projectPath: v || null })}
                placeholder={t('sidebar.taskFilters.allProjects')}
                searchPlaceholder={t('sidebar.taskFilters.searchProjects')}
                emptyMessage={t('sidebar.taskFilters.noProjects')}
                className="text-[12px]"
                resetLabel={t('sidebar.taskFilters.allProjects')}
          ...
          <div>
            <span className="mb-1.5 block text-[11px] font-semibold text-text-secondary">
              {t('sidebar.taskFilters.comments')}
            </span>
