...
export function TerminalModal({
  ...
  const resolvedTitle = title ?? t('terminal.modal.terminal');
  const resolvedSuccessMessage = successMessage ?? t('terminal.modal.completedSuccessfully');
  const resolvedFailureMessage = failureMessage ?? t('terminal.modal.processFailed');
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-border px-4 py-3">
          ...
          <button
            onClick={onClose}
            aria-label={t('terminal.modal.close')}
                    ...
                    <div className="flex flex-col">
                      ...
                      {countdown > 0 && (
                        <span className="text-xs text-text-muted">
                          {t('terminal.modal.closingIn', { count: countdown })}
                        </span>
                      )}
                    </div>
                  </div>
                    ...
                    <div className="flex flex-col">
                      <span className="text-sm font-medium text-red-400">
                        ...
                        <span className="font-mono opacity-75">
                          ({t('terminal.modal.exitCode', { code: exited })})
                        </span>
                      </span>
                      <span className="text-xs text-text-muted">
                        {t('terminal.modal.checkOutput')}
                      </span>
                    </div>
                  </div>
                )}
