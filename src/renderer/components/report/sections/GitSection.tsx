...
export const GitSection = ({ data, defaultCollapsed }: GitSectionProps) => {
  ...
  return (
    <ReportSection
      title={t('report.sections.gitActivity.title')}
      icon={GitBranch}
      defaultCollapsed={defaultCollapsed}
    >
      <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
        <div>
          <div className="text-xs text-text-muted">{t('report.sections.gitActivity.commits')}</div>
          <div className="text-sm font-medium text-text">{data.commitCount}</div>
        </div>
        <div>
          <div className="text-xs text-text-muted">{t('report.sections.gitActivity.pushes')}</div>
          <div className="text-sm font-medium text-text">{data.pushCount}</div>
        </div>
        <div>
          <div className="text-xs text-text-muted">
            {t('report.sections.gitActivity.linesAdded')}
          </div>
        ...
        <div>
          <div className="text-xs text-text-muted">
            {t('report.sections.gitActivity.linesRemoved')}
          </div>
      ...
      {data.commits.length > 0 && (
        <div>
          <div className="mb-2 text-xs font-medium text-text-muted">
            {t('report.sections.gitActivity.commits')}
          </div>
      ...
      {data.branchCreations.length > 0 && (
        <div className="mt-3">
          <div className="mb-1 text-xs font-medium text-text-muted">
            {t('report.sections.gitActivity.branchesCreated')}
          </div>
