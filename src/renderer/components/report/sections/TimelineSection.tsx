...
export const TimelineSection = ({
  ...
  return (
    <ReportSection
      title={t('report.sections.timelineActivity.title')}
      icon={Clock}
    ...
    >
      ...
      <div className="mb-4">
        <div className="mb-2 flex items-center gap-2">
          <span className="text-xs font-medium text-text-muted">
            {t('report.sections.timelineActivity.idleAnalysis')}
          </span>
      ...
      {modelSwitches.count > 0 && (
        <div className="mb-4">
          <div className="mb-2 flex items-center gap-2">
            <span className="text-xs font-medium text-text-muted">
              {t('report.sections.timelineActivity.modelSwitches')} ({modelSwitches.count})
            </span>
            ...
            {modelSwitches.switches.map((sw, idx) => (
              <div key={idx} className="flex items-center gap-2 px-2 py-0.5 text-xs">
                ...
                <span className="ml-auto text-text-muted">
                  {t('report.sections.timelineActivity.msgNumber', { number: sw.messageIndex })}
                </span>
              </div>
            ))}
          </div>
      ...
      {keyEvents.length > 0 && (
        <div>
          <div className="mb-2 text-xs font-medium text-text-muted">
            {t('report.sections.timelineActivity.keyEvents')}
          </div>
          <div className="flex flex-col gap-1">
